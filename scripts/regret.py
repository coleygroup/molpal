...
def reward_curve(
    ...
    if reward == 'scores':
        ...
        Y = np.nan_to_num(Y)        # need to rethink if scores are pos+neg

        # reward_curve = np.cumsum(Y, dtype=float)
        # reward_curve[k:] = reward_curve[k:] - reward_curve[:-k]
        # return reward_curve[k - 1:] / k

        reward_curve = np.zeros(len(Y))

        for i, score in enumerate(Y[:k]):
            heapq.heappush(heap, score)
            # top_k_avg = sum(heap) / k
            # reward_curve[i] = top_k_avg
        reward_curve[:k] = sum(heap) / k
...
def plot_regret(
        total_avgs: Iterable[np.ndarray], init_sizes: Iterable[int],
        regret: str, true_scores: Iterable[float], k: int,
    ...
    elif regret == 'top-k-ave':
        Y_t = np.empty(len(true_scores))
        Y_t[:] = np.sort(true_scores)[:k].mean()
    else:
        raise ValueError

    ax0.plot(X, Y_t, '--', c='k', lw=2, label='True')

    for total_avg, init_size, label in zip(total_avgs, init_sizes, labels):
        Y = -total_avg  # hack because i know the true data is negative
        X = np.arange(len(Y)) + 1
        R = Y - Y_t[X]
    ...
    if regret == 'total-ave':
        ...
        Y_r = np.empty(len(random_scores))
        heap = []
...
if __name__ == "__main__":
    ...
    plot_regret(
        reward_curves, init_sizes, args.regret,
