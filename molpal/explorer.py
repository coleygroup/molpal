...
class Explorer:
    ...
    Y_pred : np.ndarray
        an array parallel to the pool containing the mean predicted score
        for an input
    Y_var : np.ndarray
        an array parallel to the pool containing the variance in the predicted
    ...
    **kwargs
    ...
    def __len__(self) -> int:
        ...
        return (
            self.num_explored + self.model_updates*self.model_update_cost
        )
    ...
    @property
    def path(self) -> Path:
        """The filepath of the directory containing all automated output of 
        this Explorer"""
        return self.__path
    ...
    @property
    def budget(self) -> int:
        """the maximum budget expressed as the number of allowed objective
        evaluations"""
    ...
    def run(self):
        ...
        if self.iter == 0:
            ...
            print(f'ITER: {self.iter}/{self.max_iters} | '
    ...
    def explore_batch(self) -> float:
        ...
        inputs = self.acquirer.acquire_batch(
            ...
            explored={**self.scores, **self.failures}, k=self.k,
            cluster_ids=self.pool.cluster_ids(),
    ...
    def _update_predictions(self):
        ...
        if self.use_residuals:
            ...
            R = self.objective.residuals(self.pool.smis(), self.y_means)
            S = np.exp(s * np.random.randn(R.shape[0]))

            self.Y_var = np.abs(S*R)
            # self.y_vars = R ** 2
    ...
    def _calc_nn_uncertainty(self):
        ...
        neighbors"""
        self.Y_var = np.empty(len(self.Y_mean))
        for i, neighbors_idxs in enumerate(self.pool.neighbors):
            neighbor_y_preds = np.array([
                self.Y_mean[idx] for idx in neighbors_idxs
            ])
            self.Y_var[i] = np.var(neighbor_y_preds)
